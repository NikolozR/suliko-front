...
const SuggestionsPanel: React.FC<SuggestionsPanelProps> = ({ visible, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      <div
        className="fixed inset-0 bg-black bg-opacity-10 opacity-30 transition-opacity duration-300"
        onClick={onClose}
      />
      <div
        className={`fixed right-0 top-0 h-full w-[340px] min-w-[280px] max-w-[400px] bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-700 shadow-lg flex flex-col transition-transform duration-300 transform ${visible ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <button
          type="button"
          onClick={onClose}
          className="absolute top-2 right-2 z-10 p-2 rounded-full bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 shadow"
          title="Close"
        >
          <X className="w-5 h-5 cursor-pointer text-gray-700 dark:text-gray-300" />
        </button>
        <div className="font-semibold mb-2 text-suliko-default-color text-sm md:text-base p-4 pb-0">
          შემოთავაზებები
        </div>
        <aside className="h-full flex-1 max-h-full bg-white dark:bg-slate-900 flex flex-col overflow-hidden">

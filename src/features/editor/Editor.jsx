...
const toolbarItems = [
  ...
  "emoji", "link", "insertTable", "insertTableLayout", "highlight", "blockQuote", "|",
...
export default function Editor({ translatedMarkdown = "", onChange }) {
  ...
  // Initialize editor configuration once CKEditor Cloud is ready
  useEffect(() => {
    if (cloud.status !== "success" || !cloud.CKEditor) return;

    const { DocumentEditor, ClassicEditor, ...plugins } = cloud.CKEditor;
    const { FormatPainter } = cloud.CKEditorPremiumFeatures;

    const EditorToUse = DocumentEditor || ClassicEditor;
    if (!EditorToUse) {
      console.error("No editor available in CKEditor Cloud.");
      return;
    }
    ...
    setEditorConfig({
      EditorToUse,
      config: {
        plugins: [...Object.values(plugins), FormatPainter],
  ...
  if (cloud.status === "loading" || !editorConfig) {
    return (
      <div className="main-container">
        <div className="editor-container editor-container_document-editor p-8 text-center">
          <p>Loading editor...</p>
  ...
  return (
    <div className="main-container">
      <div className="editor-container editor-container_document-editor">
        <CKEditor
          editor={editorConfig.EditorToUse}
          ...
          onError={(error, { willEditorRestart }) => {
            console.error("CKEditor error:", error);
            if (willEditorRestart) editorInstanceRef.current = null;
          }}
        />
      </div>
    </div>
